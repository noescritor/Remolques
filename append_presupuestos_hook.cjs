...
`;
code = code.replace(/const ordenesReq = await fetchJson\('ordenes', `\$\{BASE_URL\}\/ordenes-trabajo`, token\)\.catch\(\(\) => \[\]\);\s*setOrdenesTrabajo\(Array\.isArray\(ordenesReq\) \? ordenesReq : \[\]\);/, fetchLogic);
